 ...
 */
    ...
    (() => {
        ...
        function refreshPage(breakpoints) {
            const mediaQueris = initBreakpoints();
        ...
        function fontSizeListener(func, params = false) {
            document.documentElement.addEventListener("transitionend", (e => {
                if (e.target === document.documentElement) if (e.propertyName === "font-size") params ? func(...params) : func();
            }));
        }
        const functions_slideUp = (target, duration = 500, showmore = 0) => {
        ...
        const functions_slideToggle = (target, duration = 500) => {
            if (target.hidden) return slideDown(target, duration); else return functions_slideUp(target, duration);
        };
        ...
        function functions_dataMediaQueries(array, dataSetValue) {
        ...
        function spollers() {
            ...
            if (spollersArray.length > 0) {
                ...
                let mdQueriesArray = functions_dataMediaQueries(spollersArray, "spollers");
                if (mdQueriesArray && mdQueriesArray.length) spollerіMedia(mdQueriesArray, initSpollers);
                if (openSpollersAray.length) {
                    let openQueriesArray = functions_dataMediaQueries(openSpollersAray, "openSpollers");
        ...
        function tabs() {
            ...
            if (tabs.length > 0) {
                ...
                let mdQueriesArray = functions_dataMediaQueries(tabs, "tabs");
                ...
                if (tabsContent.length > 0) {
                    ...
                    tabsContent.forEach(((tabsContentItem, index) => {
                        if (tabsTitles[index].classList.contains("_tab-active")) {
                            if (tabsBlockAnimate) slideDown(tabsContentItem, tabsBlockAnimate); else tabsContentItem.hidden = false;
        ...
        function dropdowns() {
            ...
            if (dropdownsArray.length > 0) {
                ...
                const dropdownsRegular = Array.from(dropdownsArray).filter((function(item) {
                    return !item.dataset.dropdown.split(",")[0];
                }));
                if (dropdownsRegular.length) initDropdowns(dropdownsRegular);
                let mdQueriesArray = functions_dataMediaQueries(dropdownsArray, "dropdown");
                if (mdQueriesArray && mdQueriesArray.length) mdQueriesArray.forEach((item => {
                    item.matchMedia.addEventListener("change", (function() {
                        initDropdowns(item.itemsArray, item.matchMedia);
                    }));
                    fontSizeListener(initDropdowns, [ item.itemsArray, item.matchMedia ]);
                    initDropdowns(item.itemsArray, item.matchMedia);
                }));
                ...
                function toggleDropdown(el) {
                    ...
                    if (oneDropdown && openDropdown && !dropdownBlock.classList.contains("_dropdown-open")) openDropdown.classList.remove("_dropdown-open");
        ...
        function initSliders() {
            if (document.querySelector(".main-slider")) new swiper_core_Swiper(".main-slider", {
                ...
                on: {
                    init: function(swiper) {
                        const allSlides = document.querySelector(".slider-fraction_all");
                        const currentSlide = document.querySelector(".slider-fraction_current");
            ...
            if (document.querySelector(".slider-product")) {
                ...
                new swiper_core_Swiper(".slider-product", {
        ...
        function formRating() {
            ...
            function initRatings() {
                ...
                function initRating(rating) {
                    ...
                    if (rating.classList.contains("rating_set")) setRating(); else {
                        const ratingItems = rating.querySelectorAll(".rating__item");
                        ratingItems.forEach((ratingItem => {
                            ratingItem.setAttribute("tabindex", "-1");
                        }));
                    }
                }
                ...
                function setRating() {
                    document.addEventListener("mouseover", (e => {
                        if (e.target.closest(".rating__item")) {
                            ...
                            const currentRating = e.target.closest(".rating");
                            initRatingVars(currentRating);
                            setRatingActiveWidth(ratingItem.value);
                        }
                    }));
                    document.addEventListener("mouseout", (e => {
                        if (e.target.closest(".rating__item")) setRatingActiveWidth();
                    }));
                    document.addEventListener("click", (e => {
                        if (e.target.closest(".rating__item")) {
                            ...
                            const currentRating = e.target.closest(".rating");
                            initRatingVars(currentRating);
                            if (currentRating.dataset.ajax) setRatingValue(ratingItem.value, rating); else {
                ...
                async function setRatingValue(value, rating) {
                    if (!rating.classList.contains("rating_sending")) {
                        ...
                        const server = "../../../files/rating.json";
        ...
        tippy_esm('[data-tippy-theme="main-hero"]', {
            theme: "main-hero"
        });
        tippy_esm(' [data-tippy-theme="product-tippy"]', {
            theme: "product-tuippy",
            placement: "top-end",
            offset: [ 8, 13 ]
        });
        class Popup {
            ...
            open(selectorValue) {
                ...
                if (this.targetOpen.element) {
                    ...
                    if (!this._reopen) this.options.pageLock ? pageLockToggle() : null; else this._reopen = false;
            ...
            close(selectorValue) {
                ...
                if (!this._reopen) {
                    ...
                    this.options.pageLock ? pageLockToggle(this.options.closeDuration, false) : null;
                    this.isOpen = false;
                }
                this._removeHash();
        ...
        function nouislider_unique(array) {
        ...
        function scope(target, options, originalOptions) {
            ...
            function generateSpread(pips) {
                ...
                var prevPct = 0;
                group = nouislider_unique(group.slice().sort((function(a, b) {
                    return a - b;
                })));
        ...
        function rangeInit() {
            ...
            if (rangeItems.length) rangeItems.forEach((rangeItem => {
                ...
                if (tooltips.length) tooltips.forEach((tooltip => {
                    tooltip.classList.add("_disable");
                }));
                document.addEventListener("change", (function(e) {
                    if (e.target === fromValue) item.noUiSlider.set([ e.target.value, null ]);
                    if (e.target === toValue) item.noUiSlider.set([ null, e.target.value ]);
                }));
                item.noUiSlider.on("slide", (function(values, handle) {
                    tooltips.forEach(((tooltip, index) => {
                        index === handle ? tooltip.classList.remove("_disable") : null;
                    }));
                }));
                item.noUiSlider.on("change", (function(values, handle) {
                    tooltips.forEach(((tooltip, index) => {
                        index === handle ? tooltip.classList.add("_disable") : null;
                    }));
                }));
                item.noUiSlider.on("update", (function(values, handle) {
                    handle ? toValue.value = values[handle] : fromValue.value = values[handle];
                }));
            }));
        }
        rangeInit();
        class SelectConstructor {
            ...
            getSelectTitleValue(selectItem, originalSelect) {
                ...
                if (originalSelect.hasAttribute("data-search")) return `<div class="${this.selectClasses.classSelectTitle}"><div${pseudoAttribute} class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"><svg fill="currentColor" class="icon-${this.options.selectIconName}">\n\t\t\t<use xlink:href="img/icons/icons.svg#${this.options.selectIconName}"></use>\n\t\t</svg></div></div>`; else {
                    const customClass = this.getSelectedOptionsData(originalSelect).elements.length && this.getSelectedOptionsData(originalSelect).elements[0].dataset.class ? ` ${this.getSelectedOptionsData(originalSelect).elements[0].dataset.class}` : "";
                    return `<button type="button" class="${this.selectClasses.classSelectTitle}"><div${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span><svg fill="currentColor" class="icon-${this.options.selectIconName}">\n\t\t\t<use xlink:href="img/icons/icons.svg#${this.options.selectIconName}"></use>\n\t\t</svg></div></button>`;
                }
            }
...
***************************************************************************** */        var __assign = function() {
        ...
        function toggleMenu() {
            ...
            if (!page.classList.contains("menu-slide")) {
                ...
                if (page.classList.contains("menu-open")) {
                    ...
                    const subMenuActive = document.querySelector(".sub-menu-header._sub-menu-show");
                    page.classList.contains("sub-menu-open") ? page.classList.remove("sub-menu-open") : null;
                    if (subMenuActive) {
                        subMenuActive.classList.remove("_sub-menu-show");
                        subMenuActive.querySelector("[data-submenu]._sub-menu-open").classList.remove("._sub-menu-open");
                    }
                } else {
                    pageLockToggle();
        ...
        function clickActions(e) {
            const targetElement = e.target;
            if (!targetElement.closest(".phone-dropdown__arrow") && document.querySelector("[data-dropdown]._dropdowm-open")) toggleDropdown();
            if (targetElement.closest(".sub-menu__back")) backMenu();
            if (targetElement.closest("[data-parent]")) {
                e.preventDefault();
                const targetLink = targetElement.closest("[data-parent]");
                catalogMenuToggle(targetLink);
            } else if (document.querySelector("._sub-menu-show") && !targetElement.closest(".sub-menu-header") && !targetElement.closest(".mobile-menu")) catalogMenuToggle();
            if (targetElement.closest(".icon-menu")) toggleMenu(); else if (document.documentElement.classList.contains("menu-open")) if (!targetElement.closest(".sub-menu__back") && !targetElement.closest(".mobile-menu")) toggleMenu();
        ...
        function closeCatalogMenuOnBreakpoint() {
            const matchMedia = window.matchMedia("(max-width: 991.98px)");
            matchMedia.addEventListener("change", (() => {
                document.querySelector("._sub-menu-show") ? catalogMenuToggle() : null;
                document.documentElement.classList.contains("menu-open") ? toggleMenu() : null;
            }));
        }
        closeCatalogMenuOnBreakpoint();
        function catalogMenuToggle(targetElement = document.querySelector("._sub-menu-active")) {
            const subMenuId = targetElement.dataset.parent ? targetElement.dataset.parent : null;
            const subMenu = document.querySelector(`[data-submenu="${subMenuId}"]`);
            const catalogMenu = document.querySelector(".sub-menu-header");
            if (subMenu) {
                ...
                } else catalogMenu.classList.toggle("_sub-menu-show");
                targetElement.classList.toggle("_sub-menu-active");
                if (!subMenu.classList.contains("_sub-menu-open")) subMenu.classList.add("_sub-menu-open"); else setTimeout((() => {
                    subMenu.classList.remove("_sub-menu-open");
                }), 200);
            } else console.log("subMenu not found");
        }
        function setCatalogMenuClolumsQuantiy() {
            const subMenuBlocks = document.querySelectorAll(".sub-menu-header__block");
            if (subMenuBlocks.length) {
                const subMenuBlocks = document.querySelectorAll(".sub-menu-header__block");
                subMenuBlocks.forEach((subMenuBlock => {
                    const subMenuCatalog = subMenuBlock.querySelector(".sub-menu-catalog");
                    const categotyLinks = subMenuBlock.querySelectorAll(".sub-menu-catalog__link").length;
                    const firstColumn = subMenuBlock.querySelector(".sub-menu-catalog__list");
                    firstColumn.classList.add("sub-menu-catalog__list_first-column");
                    subMenuCatalog.classList.add(`sub-menu-catalog_${categotyLinks}`);
                }));
            }
        }
